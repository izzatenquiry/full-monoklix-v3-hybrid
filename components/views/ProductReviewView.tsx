...
const ProductReviewView: React.FC<ProductReviewViewProps> = ({ onReEdit, onCreateVideo, currentUser, onUserUpdate, language }) => {
  ...
  const generateSceneImage = async (index: number, attempt = 1, excludedServers: string[] = [], initialServer?: string) => {
    ...
    const isPersonalOnly = false; // Force Hybrid

    let serverToUseUrl: string | undefined = undefined;

    if (!isPersonalOnly) {
        // Hybrid Mode Logic
        if (initialServer && !excludedServers.includes(initialServer)) {
             // 1. Prioritize initialServer if valid and not excluded (first attempt)
             serverToUseUrl = initialServer;
        } else {
             // 2. Fallback to random selection from available servers (retry or no initial)
             const availableServers = SERVERS.filter(s => !excludedServers.includes(s.url));
             if (availableServers.length > 0) {
                 const serverObj = availableServers[Math.floor(Math.random() * availableServers.length)];
                 serverToUseUrl = serverObj.url;
             } else {
                 // If all servers excluded, pick random from all (last resort)
                 const serverObj = SERVERS[Math.floor(Math.random() * SERVERS.length)];
                 serverToUseUrl = serverObj.url;
             }
        }
    ...
    try {
        const result = await editOrComposeWithImagen({
            ...
            config: { aspectRatio: '1:1', serverUrl: serverToUseUrl }
        });
        ...
        const isNonRetriable = errString.includes('[400]') || errString.toLowerCase().includes('safety');

        // Pass 'serverToUseUrl' into excluded list if retry
        const failedServer = serverToUseUrl;
        
        if (!isPersonalOnly && attempt < 3 && !isNonRetriable) {
            const newExcluded = failedServer ? [...excludedServers, failedServer] : excludedServers;
            // Hybrid retry logic - pass undefined as initialServer to force random selection
            await generateSceneImage(index, attempt + 1, newExcluded, undefined);
        } else {
            // Final failure
            const userFriendlyMessage = handleApiError(e);
            setImageGenerationErrors(prev => {
  ...
  const handleGenerateAllImages = async () => {
    ...
    // Fire parallel requests with server distribution and staggered timing
    const promises = [];
    for (let i = 0; i < 4; i++) {
        if (parsedScenes[i]) {
            // Target distinct servers for each scene initially
            const targetServer = SERVERS[i % SERVERS.length].url;
            promises.push(new Promise<void>(resolve => {
                setTimeout(async () => {
                    await generateSceneImage(i, 1, [], targetServer);
                    resolve();
                }, i * 500); // Increased to 500ms stagger per request
            }));
        }
    }
