...
const App: React.FC = () => {
  ...
  const initializeSessionData = useCallback(async (userId: string, isBackground: boolean = false) => {
    if (!userId) return;
    
    // Only show blocking spinner on initial load, NOT on background refresh
    if (!isBackground) setIsApiKeyLoading(true);
    ...
    try {
      ...
      if (masterKey) {
        ...
        if (!isBackground) console.log(`[Session] Shared master API key (...${masterKey.slice(-4)}) loaded.`);
      } else {
      ...
      if (veoTokensData && veoTokensData.length > 0) {
        ...
        if (!isBackground) console.log(`[Session] ${veoTokensData.length} VEO Auth Tokens loaded.`);
      } else {
        sessionStorage.removeItem('veoAuthTokens');
      ...
      if (!isBackground) setIsApiKeyLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (currentUser?.id) {
      ...
      initializeSessionData(currentUser.id, false);

      // Auto-refresh every 20 minutes (20 * 60 * 1000 ms) - Background (Non-blocking)
      const REFRESH_INTERVAL = 20 * 60 * 1000;
      const intervalId = setInterval(() => {
          console.log('[Auto-Refresh] Triggering scheduled token update...');
          initializeSessionData(currentUser.id, true);
      }, REFRESH_INTERVAL);
  ...
  const proceedWithPostLoginFlow = (user: User) => {
    ...
    const handleServerSelected = async (serverUrl: string) => {
        ...
        if (serverModalPurpose === 'change') {
            ...
            alert('Server changed successfully!');
            // REMOVED: window.location.reload(); // This was causing the page reload/process interruption
            setShowServerModal(false);
            // Force re-render or update of components that rely on sessionStorage
            // In this app architecture, most services read from sessionStorage on-demand,
            // so a reload isn't strictly necessary if we just updated the storage.
  ...
  const adminOnlyViews: View[] = ['api-generator', 'master-dashboard', 'token-master', 'ugc-gen'];
